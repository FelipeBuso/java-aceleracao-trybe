...
public class MedicController {
  ...
  public ResponseEntity<List<MedicDto>> getMedics() {
    ...
    //    }
    List<MedicDto> medicsDto =
        medicService.getAllMedics().stream().map(MedicDto::fromEntity).toList();
  ...
  public ResponseEntity<MedicDto> getMedicById(@PathVariable Long id) {
    ...
    return ResponseEntity.status(HttpStatus.OK).body(MedicDto.fromEntity(medic));
  }
